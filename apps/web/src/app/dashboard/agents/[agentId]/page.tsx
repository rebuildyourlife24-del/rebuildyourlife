...
import { Target, Megaphone, PenTool, Bot, Briefcase, FileLineChart, ShieldAlert } from "lucide-react";
import { notFound } from "next/navigation";

export default async function DynamicAgentPage({ params }: { params: { agentId: string } }) {
  const resolvedParams = await Promise.resolve(params);
  const agentIdLower = resolvedParams.agentId.toLowerCase();
  const agentId = agentIdLower.toUpperCase() as "HERMES" | "ORION" | "CEO" | "CFO" | "CMO" | "COO" | "SEO" | "ADS" | "CRO" | "COPY" | "DATA";
  ...
  if (!metadata) {
    // If the agent doesn't exist, Next.js will trigger the global not-found.tsx
    notFound();
  }
