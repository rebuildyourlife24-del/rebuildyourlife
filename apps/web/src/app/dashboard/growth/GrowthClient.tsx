...
import { createHabit, updateHabitHours, createBook, toggleBookRead } from '@/actions/growth';
...
export default function GrowthClient({ initialHabits, initialBooks }: { initialHabits: HobbyCategory[], initialBooks: Book[] }) {
  const [categories, setCategories] = useState<HobbyCategory[]>(initialHabits);
  ...
  const handleToggleBook = async (id: string, currentRead: boolean) => {
    // Optimistic update
    setBooks(prev => prev.map(b => b.id === id ? { ...b, read: !currentRead } : b));
    await toggleBookRead(id, !currentRead);
  };
  ...
  const handleAddHobby = async (e: React.FormEvent) => {
    ...
    // Call DB
    await createHabit(newCat);
    
    // We just reload the page to get the new id, or we could optimistic update if we had an id.
    // In a real app we'd await the createHabit and get the ID. For now we will just let Next.js revalidate via Server Actions!
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          >
            Live gesynchroniseerd met de Centrale Database.
          </motion.p>
        </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <span className="text-2xl font-bold text-emerald-400">
                {totalWeeklyGoal > 0 ? Math.round((totalWeeklyDone / totalWeeklyGoal) * 100) : 0}%
              </span>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {categories.length === 0 && <p className="text-zinc-500 text-sm">Geen habits gevonden. Voeg er een toe.</p>}
        {categories.map((cat, i) => {
          ...
          return (
            ...
            >
              <Card
                variant="glass"
                className="p-5 flex flex-col gap-4 hover:border-emerald-500/30 transition-all duration-300 h-full"
              ...
              >
                ...
                <button
                  ...
                  onClick={async () => {
                    const increment = 0.5;
                    const newValue = Math.min(cat.weeklyHoursDone + increment, cat.weeklyHoursGoal + 5);
                    // Optimistic update
                    setCategories(prev =>
                      prev.map(c => c.id === cat.id ? { ...c, weeklyHoursDone: newValue } : c)
                    );
                    await updateHabitHours(cat.id, newValue);
                  }}
                >
      ...
      >
        <Card variant="glass" className="p-6">
          ...
          <div className="mb-4">
            <ProgressBar value={booksRead} max={books.length || 1} color="gold" size="sm" />
          </div>

          <div className="space-y-2">
            {books.length === 0 && <p className="text-zinc-500 text-sm">Geen boeken op je leeslijst.</p>}
            <AnimatePresence>
              {books.map(book => (
                <motion.div
                  key={book.id}
      ...
      <Modal
        ...
        description="Voeg een nieuwe groeicategorie toe aan je leven. Dit wordt opgeslagen in je centrale database."
      >
        <form onSubmit={handleAddHobby} className="space-y-4">
            ...
            {['🎵', '🎯', '🌱', '✍️', '🚴', '🧪', '🎭', '🌍'].map(em => (
              <button
                ...
                className={`text-2xl rounded-xl p-3 transition-all ${
                  hobbyForm.emoji === em ? 'bg-emerald-500/20 border border-emerald-500/50' : 'bg-white/5 hover:bg-white/10'
                }`}
              >
                {em}
              </button>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-textSecondary">
              Weekdoel: <span className="text-emerald-400 font-bold">{hobbyForm.weeklyHoursGoal} uur</span>
            </label>
            <input
              type="range"
              min={1}
