...
export async function GET() {
  try {
    ...
    return NextResponse.json({ success: true, message: 'Academy fully seeded with Course -> Module -> Lesson structure.' });
  } catch (error: any) {
    console.error(error);
    return NextResponse.json({ error: 'Failed to seed academy: ' + error.message }, { status: 500 });
  }
}
